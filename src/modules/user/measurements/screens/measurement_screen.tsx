...
const MeasurementScreen = () => {
  ...
  const { requiredMeasurementFormFieldsLoading } = useProductsHook();
  ...
  return (
    <SafeAreaView className="h-full w-full relative flex-1 pt-[15px] bg-white">
      ...
      {(!requiredMeasurementFormFieldsLoading && !addProductToCartLoading && !addBodyMeasurementTemplateLoading) ? (
          ...
          <View className="pt-[30px] pb-5 px-[20px]">
            ...
            {requiredMeasurementFormFields?.measurements?.map((measurement, measurementIndex) => (
              <View key={measurement._id} className="h-auto w-full mt-5 px-3 py-4 relative border border-gray-200 rounded-2xl">
                ...
                <Controller
                  control={control}
                  name={`measurements[${measurementIndex}].name` as any}
                ...
                {measurement?.fields?.map((field, fieldIndex) => {
                  ...
                  return (
                    <View key={field}>
                      <Text aria-label={formattedField.label} nativeID={formattedField.name} className="mt-4 font-montserratMedium">{formattedField.label}</Text>
        ...
        (addProductToCartLoading) 
        ? <AppLoader loadingAdditionalMessage="Loading product to cart." />
        : (addBodyMeasurementTemplateLoading) 
        ? <AppLoader loadingAdditionalMessage="Creating measurement template." />
        : <AppLoader />
      )}
