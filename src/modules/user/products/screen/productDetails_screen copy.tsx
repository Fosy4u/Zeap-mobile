...
import { IImage } from '../models/productDetails_model';
...
const ProductDetailScreen: React.FC<IProps> = ({ route }) => {
  const { selectedTab, tabs, popularProducts, selectedColor } = useSelector((state: RootState) => state.productState);
  ...
  const { handleGetColorVariations } = useProductsHook();
  const { getAvailableColors } = useGeneralHook();
  ...
  const availableColors = getAvailableColors(product?.productType!, product?.variations![0].bespoke!.availableColors!);
  // console.log("AVAILABLE COLORS::: ", availableColors);
  // console.log("PRODUCT ID::: ", productID);

  useEffect(() => {
    const activeImage = product?.colors![0].images!.find((eachImage) => {
      return eachImage.isDefault === true; 
    });
    setSelectedImage(activeImage!);
  handleGetColorVariations(product?.colors!);
          ...
          >
            ...
            <View className="h-auto w-full px-[25px] flex-row items-start justify-between">
              <View className="h-auto w-[220px]">
                ...
                <Text className="mt-2  font-montserratSemiBold text-[20px] text-baseGreen">{ product!.title }</Text>
                <View className="mt-2.5 flex-row items-center">
                  <Text className="mr-5 font-montserratMedium text-baseGreen">{ product!.currency!.symbol! + product!.variations![0].price!.toLocaleString() }</Text>
            ...
            <View className="h-auto w-full mt-2 pl-[25px] flex-row items-center justify-between relative">
              ...
              { (product?.productType === "readyMadeCloth" || product?.productType === "readyMadeShoe") && (
                <View className="h-[280px] w-[150px] items-end flex justify-center relative">
                  <TouchableOpacity onPress={ () => {
                    toggleSlide();
                    console.log("OPEN DRAWER:::")
                  } }
                  className=" right-[75.5px]">
                    <Image
                  ...
                  { (availableColors.length !== 0) && (
                      <Animated.View className="h-[260px] w-[150px] pr-0 right-[75px] items-end justify-center rounded-l-full border border-[#0D986A] bg-gray-100 absolute"
                                     style={[
                      ...
                      >
                        <TouchableOpacity
                            onPress={ () => {
                              dispatch(setSelectedColor(availableColors[0]));
                              toggleSlide()
                            } }
                            className="h-7 w-7 flex items-center justify-center rounded-full absolute right-4 top-4"
                            style={ {  borderWidth: 1, borderColor: selectedColor.hex === availableColors[0]?.hex ? availableColors[0]?.hex : "transparent" } }
                        >
                          <View className="h-5 w-5 rounded-full" style={ { backgroundColor: availableColors[0]?.hex! } } />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={ () => {
                              dispatch(setSelectedColor(availableColors[1]));
                              toggleSlide()
                            } }
                            className="h-7 w-7 flex items-center justify-center rounded-full absolute left-10 top-[55px]"
                            style={ {  borderWidth: 1, borderColor: selectedColor.hex === availableColors[1]?.hex ? availableColors[1]?.hex : "transparent" } }
                        >
                          <View className="h-5 w-5 rounded-full" style={ { backgroundColor: availableColors[1]?.hex! } } />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={ () => {
                              dispatch(setSelectedColor(availableColors[2]));
                              toggleSlide()
                            } }
                            className="h-7 w-7 flex items-center justify-center rounded-full absolute left-4 top-[115px]"
                            style={ {  borderWidth: 1, borderColor: selectedColor.hex === availableColors[2]?.hex ? availableColors[2]?.hex : "transparent" } }
                        >
                          <View className="h-5 w-5 rounded-full" style={ { backgroundColor: availableColors[2]?.hex! } } />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={ () => {
                              dispatch(setSelectedColor(availableColors[3]));
                              toggleSlide()
                            } }
                            className="h-7 w-7 flex items-center justify-center rounded-full absolute left-10 bottom-[55px]"
                            style={ {  borderWidth: 1, borderColor: selectedColor.hex === availableColors[3]?.hex ? availableColors[3]?.hex : "transparent" } }
                        >
                          <View className="h-5 w-5 rounded-full" style={ { backgroundColor: availableColors[3]?.hex! } } />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={ () => {
                              dispatch(setSelectedColor(availableColors[4]));
                              toggleSlide()
                            } }
                            className="h-7 w-7 flex items-center justify-center rounded-full absolute right-4 bottom-4"
                            style={ {  borderWidth: 1, borderColor: selectedColor.hex === availableColors[4]?.hex ? availableColors[4]?.hex : "transparent" } }
                        >
                          <View className="h-5 w-5 rounded-full" style={ { backgroundColor: availableColors[4]?.hex! } } />
                        </TouchableOpacity>
            ...
            <View className="mt-3 px-5 flex-row items-center justify-start flex-wrap gap-x-2">
              { product?.colors?.length !== 0 && (product?.colors?.map((eachColor, index) => (
                <TouchableOpacity  key={ eachColor._id } 
                  onPress={ () => {
                    setSelectedImage(eachColor!.images![index]!);
                    console.log("SELECTED IMAGE: ", eachColor?.images?.[index]!);
                    console.log("ACTIVE IMAGE: ", selectedImage._id);      
                  }}
                  className={`h-[75px] w-[75px] rounded-2xl border ${ (eachColor!.images![index]?._id! === selectedImage?._id!) ? "border-baseGreen" : "border-gray-300" } bg-[#F8F9FE]`}>
                  <Image
                    source={
            ...
            <View className="mt-5 px-5 pt-5 bg-lightGray">

              {/*==== Available Colours ====*/}
              <View className="h-auto w-full">
                <Text className="font-semibold text-gray-700 text-[15px]">Available Cclours</Text>
                <View className="h-auto w-full mt-4 flex-row gap-x-4 items-center justify-start flex-wrap">
                  { availableColors.map((eachColor, index) => (
                    <TouchableOpacity key={ index }
                      onPress={ () => dispatch(setSelectedColor(eachColor)) }
