...
const MeasurementScreen = () => {
  ...
  const [showSavedMeasurementBottomSheet, setShowSavedMeasurementBottomSheet] = useState(false);
  const unitOptions = [
      { "key": "in", "value": "in" },
      { "key": "cm", "value": "cm" },
  ];

  const handleShowSavedMeasurementBottomSheet = (value: boolean) => {
    setShowSavedMeasurementBottomSheet(value);
  };
  
  return (
    <SafeAreaView className="h-full w-full relative flex-1 pt-[15px] bg-white">
      <StatusBar
      ...
      <ScrollView showsVerticalScrollIndicator={ false }>
        <View className="pt-[30px] pb-5 px-[20px]">
          ...
          <TouchableOpacity 
            onPress={ () => handleShowSavedMeasurementBottomSheet(true) }
            className="h-[55px] w-full mt-6 flex-row items-center justify-center rounded-xl bg-lightGold"
          >
            <Text className="font-montserratMedium text-base text-green-800">Apply Existing Measurement</Text>
          </TouchableOpacity>

          <Text className="mt-4 font-montserratSemiBold text-center">OR</Text>

          <Text className="mt-3 font-montserratSemiBold text-base text-center text-gray-700">Enter new measurement</Text>

          <View className="h-auto w-full mt-5 px-3 py-4 relative border border-gray-200 rounded-2xl">
            <Text className="mt-2 font-montserratSemiBold text-base text-gray-700">Blazers</Text>
            <View className="h-auto w-[60px] absolute p-2 top-4 right-3 rounded-xl border border-gray-200 bg-gray-50 z-50">
              <SelectList
      ...
      { showSavedMeasurementBottomSheet && (
        <SavedMeasurementsBottomSheet handleShowSavedMeasurementBottomSheet={ handleShowSavedMeasurementBottomSheet } />
      ) }
