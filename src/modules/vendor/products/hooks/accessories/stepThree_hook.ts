...
import { useLazyGetProductByProductIDQuery, useUpdateProductMutation } from "../../apis/accessoryProduct_api";
...
const useStepThreeHook = () => {
    ...
    const handleGetProductSizeMeasurements = async () => {
        if (!accessoriesOptions) return;
        
        const accessorySizes = accessoriesOptions.accessorySizeEnums!;
        setAccessorySizes(accessorySizes);
    };
    ...
    useEffect(() => {
        if (product) {
            handleGetProductSizeMeasurements();
        }
    }, [accessoriesOptions]);
    useEffect(() => {
        handleSelectSizes();
    ...
    return {
        accessorySizes, selectedSizes, setSelectedSizes,
        handleSubmit,
    };
};
