...
const AllCategoryScreen = () => {
  const { categories } = useSelector((state: RootState) => state.homeState);
        ...
        <ScrollView showsVerticalScrollIndicator={false} className="mt-3">
          { categories.map((category) => (
            <TouchableOpacity key={ category.id }
              onPress={ () => navigation.navigate("categoryListScreen", { screenTitle: category.name }) }>
              ...
              >
                <View>
                  ...
                  <Text className="text-xs text-gray-600">{ category.itenCount } items</Text>
                </View>
